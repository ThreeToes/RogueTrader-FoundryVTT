export class DarkHeresyItem extends Item {
	...
	get Part() {
		...
		let parts = [];
		if (part.head > 0)
			parts.push(`${game.i18n.localize("ARMOUR.HEAD")} (${part.head})`);
		if (part.leftArm > 0)
			parts.push(`${game.i18n.localize("ARMOUR.LEFT_ARM")} (${part.leftArm})`);
		if (part.rightArm > 0)
			parts.push(
				`${game.i18n.localize("ARMOUR.RIGHT_ARM")} (${part.rightArm})`,
			);
		if (part.body > 0)
			parts.push(`${game.i18n.localize("ARMOUR.BODY")} (${part.body})`);
		if (part.leftLeg > 0)
			parts.push(`${game.i18n.localize("ARMOUR.LEFT_LEG")} (${part.leftLeg})`);
		if (part.rightLeg > 0)
			parts.push(
				`${game.i18n.localize("ARMOUR.RIGHT_LEG")} (${part.rightLeg})`,
			);
		return parts.join(" / ");
	}
	...
	get isAbilities() {
		return this.isTalent || this.isTrait || this.isSpecialAbility;
	}
	...
	get weightSum() {
		return this.system.quantity * this.system.weight;
	}
