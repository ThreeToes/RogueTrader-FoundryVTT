export const migrateWorld = async () => {
	...
	const worldSchemaVersion = Number(
		game.settings.get("rogue-trader", "worldSchemaVersion"),
	);
	if (worldSchemaVersion !== schemaVersion && game.user.isGM) {
		...
		for (let pack of game.packs.filter(
			(p) =>
				p.metadata.package === "world" && ["Actor"].includes(p.metadata.type),
		)) {
...
const migrateActorData = (actor, worldSchemaVersion) => {
	...
	if (worldSchemaVersion < 1) {
		if (actor.data.type === "acolyte" || actor.data.type === "npc") {
			...
			update["system.skills.psyniscience"] =
				actor.data.data.skills.psyniscience;
		}
	}
	if (worldSchemaVersion < 2) {
		if (actor.data.type === "acolyte" || actor.data.type === "npc") {
			...
			update["system.skills.forbiddenLore"] =
				actor.data.data.skills.forbiddenLore;
		}
	}
	...
	if (worldSchemaVersion < 4) {
		if (actor.data.type === "acolyte" || actor.data.type === "npc") {
			...
			if (textAptitudes !== null && textAptitudes !== undefined) {
				let aptitudeItemsData = Object.values(textAptitudes)
