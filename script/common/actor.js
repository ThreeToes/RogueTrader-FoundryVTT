export class DarkHeresyActor extends Actor {
	...
	_computeItems() {
		...
		for (let item of this.items) {
			if (item.weight) {
				encumbrance =
					encumbrance + (item.quantity ? item.weightSum : item.weight);
			}
		}
	...
	_computeExperience_auto() {
		...
		let characterAptitudes = this.items
			.filter((it) => it.isAptitude)
			.map((it) => it.name.trim());
		...
		let psyRatingCost = Math.max(
			0,
			((this.psy.rating * (this.psy.rating + 1)) / 2 - 1) * 200,
		); // N*(n+1)/2 equals 1+2+3... -1 because we start paying from 2

		this.psy.cost = this.experience.spentPsychicPowers = psyRatingCost;
		for (let characteristic of Object.values(this.characteristics)) {
			let matchedAptitudes = characterAptitudes.filter((it) =>
				characteristic.aptitudes.includes(it),
			).length;
			let cost = 0;
			for (
				let i = 0;
				i <= characteristic.advance / 5 &&
				i <= config.characteristicCosts.length;
				i++
			) {
		...
		for (let skill of Object.values(this.skills)) {
			let matchedAptitudes = characterAptitudes.filter((it) =>
				skill.aptitudes.includes(it),
			).length;
			if (skill.isSpecialist) {
				for (let speciality of Object.values(skill.specialities)) {
					let cost = 0;
					for (
						let i = speciality.starter ? 1 : 0;
						i <= speciality.advance / 10;
						i++
					) {
		...
		for (let item of this.items.filter(
			(it) => it.isTalent || it.isPsychicPower,
		)) {
			if (item.isTalent) {
				let talentAptitudes = item.aptitudes.split(",").map((it) => it.trim());
				let matchedAptitudes = characterAptitudes.filter((it) =>
					talentAptitudes.includes(it),
				).length;
				let cost = 0;
	...
	hasCondition(conditionKey) {
		let existing = this.effects.find(
			(i) => i.getFlag("core", "statusId") === conditionKey,
		);
		return existing;
	}
