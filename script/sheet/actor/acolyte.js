...
export class AcolyteSheet extends DarkHeresySheet {
	static get defaultOptions() {
		return foundry.utils.mergeObject(super.defaultOptions, {
			...
			template: "systems/rogue-trader/template/sheet/actor/acolyte.hbs",
			width: 700,
			height: 881,
			tabs: [
				{
	...
	activateListeners(html) {
		...
		html.find(".aptitude-create").click(async (ev) => {
			await this._onAptitudeCreate(ev);
		});
		html.find(".aptitude-delete").click(async (ev) => {
			await this._onAptitudeDelete(ev);
		});
		html.find(".item-cost").focusout(async (ev) => {
			await this._onItemCostFocusOut(ev);
		});
		html.find(".item-starter").click(async (ev) => {
			await this._onItemStarterClick(ev);
		});
	}
