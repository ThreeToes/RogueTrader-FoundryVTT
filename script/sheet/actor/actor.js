...
export class DarkHeresySheet extends foundry.appv1.sheets.ActorSheet {
	activateListeners(html) {
		...
		html
			.find(".roll-characteristic")
			.click(async (ev) => await this._prepareRollCharacteristic(ev));
		html
			.find(".roll-skill")
			.click(async (ev) => await this._prepareRollSkill(ev));
		html
			.find(".roll-speciality")
			.click(async (ev) => await this._prepareRollSpeciality(ev));
		html
			.find(".roll-insanity")
			.click(async (ev) => await this._prepareRollInsanity(ev));
		html
			.find(".roll-corruption")
			.click(async (ev) => await this._prepareRollCorruption(ev));
		html
			.find(".roll-weapon")
			.click(async (ev) => await this._prepareRollWeapon(ev));
		html
			.find(".roll-psychic-power")
			.click(async (ev) => await this._prepareRollPsychicPower(ev));
	...
	async _enrichment() {
		...
		if (this.actor.type !== "npc") {
			enrichment["system.bio.notes"] = await TextEditor.enrichHTML(
				this.actor.system.bio.notes,
				{ async: true },
			);
		} else {
	...
	_onAmmoUnlink(event) {
		event.preventDefault();
		const ammoId = $(event.currentTarget)
			.parents(".linked-item")
			.data("ammoId");
		const weaponId = $(event.currentTarget).parents(".item").data("itemId");

		let newAmmos = this.actor.items
			.get(weaponId)
			.system.ammo.filter((ammo) => ammo !== ammoId);
	...
	async _prepareRollCharacteristic(event) {
		...
		await prepareCommonRoll(
			DarkHeresyUtil.createCharacteristicRollData(
				this.actor,
				characteristicName,
			),
		);
	...
	async _prepareRollSpeciality(event) {
		...
		await prepareCommonRoll(
			DarkHeresyUtil.createSpecialtyRollData(
				this.actor,
				skillName,
				specialityName,
			),
	...
	async _prepareRollInsanity(event) {
		event.preventDefault();
		await prepareCommonRoll(DarkHeresyUtil.createFearTestRolldata(this.actor));
	}
	...
	_isCondition(effect) {
		return CONFIG.statusEffects
			.map((i) => i.id)
			.includes(Array.from(effect.statuses)[0]);
	}
}
