...
export async function combatRoll(rollData) {
	if (
		rollData.weapon.traits.spray &&
		game.settings.get("rogue-trader", "useSpraytemplate")
	) {
		let template = PlaceableTemplate.cone(
			{ item: rollData.itemId, actor: rollData.ownerId },
			30,
			rollData.weapon.range,
		);
...
async function _computeCombatTarget(rollData) {
	...
	let psyModifier = 0;
	if (
		typeof rollData.psy !== "undefined" &&
		typeof rollData.psy.useModifier !== "undefined" &&
		rollData.psy.useModifier
	) {
...
async function _computeCommonTarget(rollData) {
	if (rollData.flags.isEvasion) {
		...
		switch (rollData.evasions.selected) {
			case "dodge":
				skill = rollData.evasions.dodge;
				break;
			case "parry":
				skill = rollData.evasions.parry;
				break;
			case "deny":
				skill = rollData.evasions.deny;
				break;
		}
		rollData.target.final = _getRollTarget(
			rollData.target.modifier,
			skill.target.base,
		);
	} else {
...
async function _rollDamage(rollData) {
	...
	if (rollData.weapon.damageFormula) {
		...
		if (rollData.weapon.traits.proven) {
			formula = _appendNumberedDiceModifier(
				formula,
				"min",
				rollData.weapon.traits.proven,
			);
		}
		if (rollData.weapon.traits.primitive) {
			formula = _appendNumberedDiceModifier(
				formula,
				"max",
				rollData.weapon.traits.primitive,
			);
		}
	...
	let minDamage = rollData.damages.reduce(
		(min, damage) => (min.minDice < damage.minDice ? min : damage),
		rollData.damages[0],
	);
...
function _computeNumberOfHits(
	...
	let hits =
		(1 + Math.floor((attackDos - 1) / attackType.hitMargin)) * stormMod;
		...
		if (numDice >= 1) {
			...
			ar.terms
				...
				.forEach(async (die) => {
					...
					if (
						die.active &&
						(typeof damage.minDice === "undefined" ||
							die.result < damage.minDice)
					)
						damage.minDice = die.result;
				});
...
async function _updateRangedAmmo(rollData) {
	let firerate = 1;
	let mod =
		rollData.weapon.traits.storm || rollData.weapon.traits.twinLinked ? 2 : 1;
...
async function _rollPenetration(rollData) {
	let penetration = rollData.weapon.penetrationFormula
		? _replaceSymbols(rollData.weapon.penetrationFormula, rollData)
		: "0";
	let multiplier = 1;
...
function _computePsychicPhenomena(rollData) {
	rollData.psy.hasPhenomena = rollData.psy.push
		? !_isDouble(rollData.result)
		: _isDouble(rollData.result);
}
...
function _appendTearing(formula) {
	let diceRegex = /\d+d\d+/;
	if (!formula.match(/dl|kh/gi, formula)) {
		// Already has drop lowest or keep highest
