				...
				roll: {
					...
					callback: async (html) => {
						if (rollData.flags?.isEvasion) {
							...
							if (skill) {
								rollData.name = game.i18n.localize(
									skill.options[skill.selectedIndex].text,
								);
								rollData.evasions.selected = skill.value;
							}
						} else {
							rollData.name = game.i18n.localize(rollData.name);
							rollData.target.base = parseInt(
								html.find("#target")[0].value,
								10,
							);
							rollData.rolledWith = html
								.find("[name=characteristic] :selected")
								.text();
						}
						rollData.target.modifier = parseInt(
							html.find("#modifier")[0].value,
							10,
						);
					...
					roll: {
						...
						callback: async (html) => {
							...
							if (rollData.weapon.traits.inaccurate) {
								...
								rollData.weapon.traits.accurate &&
							...
							rollData.weapon.damageBonus = parseInt(
								html.find("#damageBonus")[0].value,
								10,
							);
			...
			buttons: {
				roll: {
					...
					callback: async (html) => {
						...
						rollData.weapon.damageFormula =
							html.find("#damageFormula")[0].value;
						rollData.weapon.damageType = html.find("#damageType")[0].value;
						rollData.weapon.damageBonus = parseInt(
							html.find("#damageBonus")[0].value,
							10,
						);
						rollData.weapon.penetrationFormula =
							html.find("#penetration")[0].value;
						...
						rollData.attackType.text =
							attackType.options[attackType.selectedIndex].text;
